...
const Device = function (config, Model) {
  ...
  let is509 = false;
  if (config.connectionString.indexOf('509') > -1) is509 = true;
  if (config.options.key && config.options.cert) is509 = true;
  if (is509) {
  ...
  let initializeHubClient = () => {
    ...
    */
    ...
    hubClient.setOptions(config.options, (err) => {
      if (err) log.error('SetOptions Error: ' + err);
      else hubClient.open(() => self.emit('loop-on'));
    });
